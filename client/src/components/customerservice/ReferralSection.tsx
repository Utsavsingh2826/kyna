...
export default function ReferralSection({ isOpen }: ReferralSectionProps) {
    ...
    const fetchUserData = async () => {
      try {
        ...
        const historyResponse = await axios.get(
          `${API_CONFIG.BASE_URL}/referrals/my-referrals`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const handleReferralSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Sending referral request to:', `${API_CONFIG.BASE_URL}/referrals`);
      console.log('Request payload:', { toEmails, note: referralForm.note, sendReminder: referralForm.sendReminder });
      console.log('Token present:', !!token);

      const response = await axios.post(
        `${API_CONFIG.BASE_URL}/referrals`,
        {
          toEmails,
