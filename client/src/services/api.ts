...
class ApiService {
    ...
    try {
      ...
      if (response.ok) {
        ...
        return {
          ...
          ...data, // Pass through all additional fields from backend
        };
      }
    } catch {
      return {
  ...
  async updateCartPromoCode(promoCode: string | null) {
    return this.makeRequest("/cart/promo-code", {
      method: "PUT",
  ...
  async applyReferralCode(code: string, subtotal: number) {
    return this.makeRequest("/v1/referral-code/apply", {
      method: "POST",
      body: JSON.stringify({ code, subtotal }),
    });
  }
  ...
  // Billing info management methods
  async getUserBillingInfo() {
    return this.makeRequest("/address/billing-info", {
      method: "GET",
    });
  }
  ...
  // Legacy address management methods (for backward compatibility)
