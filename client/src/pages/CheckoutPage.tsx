...
const CheckoutPage = () => {
  ...
  const [billingServiceabilityStatus, setBillingServiceabilityStatus] =
    useState<"idle" | "checking" | "serviceable" | "not-serviceable">("idle");
  const [billingServiceabilityMessage, setBillingServiceabilityMessage] =
    useState<string>("");
  const [shippingServiceabilityStatus, setShippingServiceabilityStatus] =
    useState<"idle" | "checking" | "serviceable" | "not-serviceable">("idle");
  const [shippingServiceabilityMessage, setShippingServiceabilityMessage] =
    useState<string>("");
  ...
  // Initialize persistent order ID from location state or generate new one
  useEffect(() => {
    const orderData = location.state?.orderData;
    if (orderData?.orderId && !persistentOrderId) {
      console.log(
        "📋 Using existing order ID from navigation state:",
        orderData.orderId
      );
      setPersistentOrderId(orderData.orderId);
      setExistingOrderData(orderData);
    } else if (!persistentOrderId) {
      // Generate new order ID only if none exists
  ...
  }, [location.state, persistentOrderId]);
  ...
  const handleShipToBillingAndSave = async () => {
    if (formData.shipToBilling) {
      ...
      try {
        ...
        const billingResponse = await apiService.updateBillingAddress(
          billingData
        );
        ...
        const shippingResponse = await apiService.updateShippingAddress(
          shippingData
        );
  ...
  const paymentMethods = [
    { id: "cod", name: "Cash on Delivery", icon: DollarSign },
    {
      id: "venmo",
      name: "Venmo",
      icon: () => <span className="text-blue-600 font-bold">V</span>,
    },
    {
      id: "paypal",
      name: "Paypal",
      icon: () => <span className="text-blue-600 font-bold">PP</span>,
    },
    {
      id: "amazon",
      name: "Amazon Pay",
      icon: () => <span className="text-orange-600 font-bold">a</span>,
    },
  ...
  const checkServiceability = async (
    ...
    try {
      ...
      console.debug(
        "🔎 Raw serviceability status from API (checkout):",
        statusRaw,
        typeof statusRaw
      );
      const statusStr =
  ...
  const handleProceedToCheckout = async () => {
    ...
    if (billingServiceabilityStatus !== "serviceable") {
      if (billingServiceabilityStatus === "not-serviceable") {
        ...
        console.log(
          "📍 Checking serviceability for billing pincode:",
          formData.zipCode
        );
        const isBillingServiceable = await checkServiceability(
          formData.zipCode,
          "billing"
        );
    ...
    if (!formData.shipToBilling && formData.shippingZipCode) {
      ...
      if (shippingServiceabilityStatus !== "serviceable") {
        if (shippingServiceabilityStatus === "not-serviceable") {
          ...
          console.log(
            "📍 Checking serviceability for shipping pincode:",
            formData.shippingZipCode
          );
          const isShippingServiceable = await checkServiceability(
            formData.shippingZipCode,
            "shipping"
          );
    ...
    console.log(
      "✅ Both billing and shipping areas are serviceable, proceeding with checkout..."
    );
    ...
    try {
      ...
      const shippingAddressData = {
        companyName: formData.shipToBilling
          ? formData.companyName
          : formData.shippingCompanyName,
        street: formData.shipToBilling
          ? formData.address
          : formData.shippingAddress,
        city: formData.shipToBilling ? formData.city : formData.shippingCity,
        state: formData.shipToBilling
          ? formData.region
          : formData.shippingRegion,
        country: formData.shipToBilling
          ? formData.country
          : formData.shippingCountry,
        zipCode: formData.shipToBilling
          ? formData.zipCode
          : formData.shippingZipCode,
        sameAsBilling: formData.shipToBilling,
      };
      ...
      console.log("Payment data validation:", {
        ...
        billingInfoFields: paymentData.billingInfo
      ...
      if (error.message) {
        if (error.message.includes("Missing required fields")) {
          ...
          console.log(
            "⚠️ Order already exists, attempting to fetch existing order:",
            persistentOrderId
          );

          try {
            // Try to fetch the existing order details
            const existingOrderResponse = await apiService.getOrder(
              persistentOrderId
            );

            if (existingOrderResponse.success && existingOrderResponse.data) {
              console.log(
                "✅ Found existing order, proceeding with payment:",
                existingOrderResponse.data
              );

              // Continue with existing order for payment
              const response = await paymentService.initiatePayment({
                ...paymentData,
                orderId: persistentOrderId,
                existingOrder: true,
              });

              if (response.success) {
                console.log(
                  "✅ Payment re-initiated successfully with existing order"
                );

                // Proceed with Razorpay checkout using existing order
                const razorpayOptions = {
                  key: response.data.razorpayKeyId,
                  amount: response.data.amount,
                  currency: response.data.currency,
                ...
                paymentService.openRazorpayCheckout(
                  ...
                  async (paymentResponse) => {
                    try {
                      console.log(
                        "Payment successful with existing order:",
                        paymentResponse
                      );
                      ...
                      if (verificationResult.success) {
                        console.log(
                          "✅ Payment verified successfully for existing order"
                        );

                        navigate("/order-success", {
                          state: {
                            order: existingOrderResponse.data,
                            orderNumber: persistentOrderId,
                            paymentId: paymentResponse.razorpay_payment_id,
                          },
                        });
                      } else {
            ...
            // If fetching existing order fails, show error
            errorMessage =
              "Unable to process payment with existing order. Please refresh the page and try again.";
          } catch (fetchError) {
            console.error("Error fetching existing order:", fetchError);
            errorMessage =
              "Order ID conflict detected. Please refresh the page and start again.";
          }
        } else if (
          error.message.includes("exceeds the maximum allowed limit")
        ) {
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select
                        ...
                        onChange={(e) =>
                          handleSelectChange("shippingCity", e.target.value)
                        }
                        className="w-full p-3 border border-gray-300 rounded-md appearance-none bg-white pr-10"
                        disabled={!formData.shippingRegion}
                      >
                        <option value="">Select...</option>
                        {getCitiesByState(formData.shippingRegion).map(
                          (city) => (
                            <option key={city} value={city}>
                              {city}
                            </option>
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          setFormData((prev) => ({
                            ...prev,
                            saveCardDetails: checked as boolean,
                          }))
                        }
                      />
                      <label
