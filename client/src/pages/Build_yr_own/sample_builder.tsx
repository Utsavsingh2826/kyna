          ...
          if (chosenVariant) {
            ...
            if (parts.length >= 2) {
              ...
              if (parts.length >= 5) {
                ...
                // Find matching display clarity from filtered options
                // We don't have direct access to 'data' in helpers unless we pass it or use state, 
                // but here 'data' IS available in scope.
...
if (data.deliveryDays) {
  ...
  const generateVariantId = (substyle: SubStyle) => {
    ...
    // Add bracelet size if selected (bracelets have size as last part)
    if (selectedSize) {
      // Extract size number from selectedSize (e.g., "6 (16MM)" -> "6")
      const sizeMatch = selectedSize.match(/^(\d+)/);
      const sizeCode = sizeMatch ? sizeMatch[1] : selectedSize;
      variantId += `-${sizeCode}`;
    }
  ...
  const getAvailableColorClarities = useCallback(() => {
    ...
    // Fallback to old behavior with filtering
  ...
  useEffect(() => {
    if (selectedStyleData?.productDetails) {
      ...
      // Set default selections from API data ONLY if values are empty or invalid
  ...
  useEffect(() => {
    ...
    if (!selectedDiamondSize || !selectedGoldKarat || !selectedDiamondShape || !selectedSize) {
      return;
    }
