 ...
 // Handle zip code change with real-time serviceability check
        ...
        rightColumn={
          <div className="space-y-6">
            ...
            <div>
              ...
              {/* Bracelet/Bangles Size (Inches or Ani) */}
                        <div>
                          <label className="text-sm text-muted-foreground">
                            Bracelet/Bangles Size (Inches or Ani)
                          </label>
  ...
  const handleEngravingSaved = async (
    ...
    try {
      ...
      const customizationRequestData = {
        ...
        subCategory: formData.jewelryType === 'bracelet' ? 'Custom Bracelets' : `Custom ${formData.jewelryType}`,
      ...
      if (uploadedFiles.length > 0 || engravingBlobs.length > 0) {
        ...
        // Add engraved blob if it exists
        if (engravingBlobs.length > 0) {
          engravingBlobs.forEach((engravingItem, index) => {
            const engravingFile = new File([engravingItem.blob], `engraved-image-${index + 1}.png`, {
              type: "image/png",
              lastModified: new Date().getTime(),
            });
            imageFormData.append("images", engravingFile);
            console.log(`🎨 Adding engraved image ${index + 1}:`, {
        ...
        if (imageResult.success && imageResult.data?.images) {
          ...
          console.log(`📊 Total images uploaded: ${uploadedImageUrls.length} (reference: ${uploadedFiles.length}, engraved: ${engravingBlobs.length})`);
        } else {
  ...
  return (
    <>
      ...
      {/* Engraving Modal - Rendered Outside Main Container for Proper Z-Index */}
      {showEngravingPopup && (
        <EngravingPage
