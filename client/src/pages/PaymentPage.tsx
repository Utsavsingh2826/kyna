...
const PaymentPage = () => {
  ...
  const orderData = {
    ...
    images: cart.items.flatMap((item: any) => {
      const images = [];
      ...
      if (item.product?.images?.sub && Array.isArray(item.product.images.sub)) {
        images.push(...item.product.images.sub.map((url: string) => ({
          url,
          alt: item.product?.title
        })));
      }
        ...
        <div className="mb-8">
          ...
          <div className="bg-white rounded-lg p-6 mb-6">
            ...
            <p className="text-gray-600">
              ...
              Total: <span className="font-semibold text-gray-900">₹{(cart.totalAmount || 0).toLocaleString()}</span>
            </p>
          </div>
        </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg p-6 sticky top-4 shadow-sm border border-gray-200">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm text-gray-600">
                  <span>Tax (18%)</span>
                  <span>₹{(Math.round((cart.totalAmount || 0) * 0.18)).toLocaleString()}</span>
                </div>
                <div className="border-t pt-3 mt-3">
                  <div className="flex justify-between font-semibold text-gray-900">
                    <span>Total</span>
                    <span className="text-lg text-teal-600">
                      ₹{(Math.round((cart.totalAmount || 0) * 1.18)).toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
