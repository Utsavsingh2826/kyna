  ...
  cron.schedule(
    ...
    async () => {
      ...
      try {
        ...
        for (const order of orders) {
          try {
            ...
            if (previousStatus !== newStatus) {
              console.log(
                `✅ Order ${order.orderNumber}: ${previousStatus} → ${newStatus}`
              );
        ...
        console.log(
          `🎉 Tracking update completed: ${updatedCount} orders updated, ${errorCount} errors`
        );
      } catch (error) {
...
export const runTrackingUpdateJob = async (
