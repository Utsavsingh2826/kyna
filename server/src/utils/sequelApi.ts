...
// Use production or test endpoint based on environment
const getBaseUrl = (): string => {
  const base = process.env.NODE_ENV === 'production' 
    ? process.env.SEQUEL247_PROD_ENDPOINT 
    : (process.env.SEQUEL247_TEST_ENDPOINT || process.env.SEQUEL_BASE_URL);
...
const TOKEN = process.env.NODE_ENV === 'production'
  ...
  : (process.env.SEQUEL247_TEST_TOKEN || process.env.SEQUEL_API_TOKEN);

const headers = {
  'Content-Type': 'application/json'
  // Note: Token goes in request body, not Authorization header
};
...
export interface CreateAddressPayload {
  ...
  pinCode: number; // 6 digit pincode
  auth_receiver_name: string; // Person who will receive pickups
  auth_receiver_phone: number; // 10 digit phone number
...
export interface CreateShipmentPayload {
  ...
  pickUpDate?: 'Today' | 'Tomorrow' | string; // YYYY-MM-DD format (optional)
  pickUpTime?: string; // e.g., "9:00-10:00" (optional)
  ...
  gross_weight?: string; // Weight in grams (including packaging) - optional
...
export interface CreateShipmentResponse {
  ...
  message?: string;
  data?: {
    docket_number: string; // ⭐ Main tracking number (note: underscore, not camelCase)
    brn: string; // Booking reference number
    estiimated_delivery?: string; // "DD-MM-YYYY" (note: typo in API "estiimated")
    estimated_delivery?: string; // Alternative spelling
    senders_name?: string;
...
/**
 * Track Multiple Shipments API Response
 * Based on actual Sequel247 API response structure
 */
export interface TrackMultipleResponse {
  ...
  successShipments?: {
    [docketNumber: string]: {
      ...
      shipment_status?: string; // e.g., "SCREATED", "SDELVD"
      tracking: TrackingEvent[]; // Array of tracking events
    };
  };
  errorShipments?: {
    [docketNumber: string]: {
...
export interface CancelShipmentResponse {
  status: string; // "true" or "false"
  message?: string; // e.g., "Shipment is cancelled successfully"
  errorInfo?: any[]; // Array of error information (usually empty on success)
}

/**
 * POD Download API - Proof of Delivery
 * Based on actual Sequel247 API format
 */
...
export interface PODDownloadResponse {
  status: string; // "true" or "false"
  link?: string; // Direct download link (zip file URL)
  message?: string; // e.g., "Successfully created file POD zip."
  errorInfo?: any[]; // Array of error information (usually empty on success)
}
...
export const createKynaShipment = async (params: {
  ...
  grossWeight?: number; // in grams (optional)
  orderValue: number; // in INR
  codAmount?: number; // COD amount in INR
  invoiceNumbers?: string[];
  pickupDate?: 'Today' | 'Tomorrow' | string;
  pickupTime?: string;
  remark?: string;
}) => {
  ...
  const payload: CreateShipmentPayload = {
    ...
    remark: params.remark || 'Custom jewelry - Handle with care'
  };
  
  // Add optional fields only if provided
  if (params.pickupDate) {
    payload.pickUpDate = params.pickupDate;
  }
  if (params.pickupTime) {
    payload.pickUpTime = params.pickupTime;
  }
  if (params.grossWeight) {
    payload.gross_weight = params.grossWeight.toString();
  }
...
export const createAddress = async (
  ...
  try {
    ...
    const body = {
      ...
      auth_receiver_phone: payload.auth_receiver_phone.toString()
    };
...
export const createSequelShipment = async (
  ...
  try {
    ...
    const requestBody: any = {
      ...
      fromStoreCode: payload.fromStoreCode,
      toAddress: payload.toAddress,
      net_weight: payload.net_weight,
      net_value: payload.net_value,
      codValue: payload.codValue || '0',
    ...
    if (payload.pickUpDate) {
      requestBody.pickUpDate = payload.pickUpDate;
    }
    if (payload.pickUpTime) {
      requestBody.pickUpTime = payload.pickUpTime;
    }
    if (payload.gross_weight) {
      requestBody.gross_weight = payload.gross_weight;
    }
    if (payload.boxes && payload.boxes.length > 0) {
      requestBody.boxes = payload.boxes;
    }
    if (payload.remark) {
      requestBody.remark = payload.remark;
    }
    
    const res: AxiosResponse<CreateShipmentResponse> = await axios.post(url, requestBody, { headers });
    return res.data;
  } catch (error: any) {
 ...
 * 
 * @param dockets Array of docket numbers (10-digit strings)
 * @returns Tracking information for all shipments with successShipments and errorShipments
 */
export const trackMultiple = async (dockets: string[] = []): Promise<TrackMultipleResponse> => {
  try {
    if (!dockets || dockets.length === 0) {
      return {
        status: 'false',
        message: 'Docket numbers array is required'
      };
    }

    const url = `${BASE}/api/trackMultiple`;
    
    // Request format: { token, dockets: [...] }
    const requestBody = {
      token: TOKEN,
      dockets: dockets
    };
 ...
 * @returns POD download link (zip file URL)
 */
export const podDownload = async (payload: PODDownloadPayload): Promise<PODDownloadResponse> => {
  try {
    ...
    // Add dockets if provided
    if (dockets && dockets.length > 0) {
      requestBody.dockets = dockets;
    }

    // Add date range if provided
    if (fromDate) {
      requestBody.fromDate = fromDate; // Format: YYYY-MM-DD
    }
    if (toDate) {
      requestBody.toDate = toDate; // Format: YYYY-MM-DD
    }
...
export const validateSequelConfig = (): boolean => {
  if (!BASE || !TOKEN) {
    ...
    if (process.env.NODE_ENV === 'production') {
      console.error('   Please set SEQUEL247_PROD_ENDPOINT and SEQUEL247_PROD_TOKEN in .env file');
    } else {
      console.error('   Please set SEQUEL247_TEST_ENDPOINT and SEQUEL247_TEST_TOKEN (or SEQUEL_BASE_URL and SEQUEL_API_TOKEN) in .env file');
    }
    return false;
  }
  return true;
