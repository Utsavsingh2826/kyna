...
const referralSchema = new Schema<IReferral>(
  {
    referFrdId: {
      type: String,
      required: true,
      index: true, // Keep indexed for fast lookups, but NOT unique
    },
    fromUserId: {
...
// referFrdId index already defined in schema above
referralSchema.index({ fromUserId: 1 });
referralSchema.index({ status: 1 });
referralSchema.index({ expiresAt: 1 });
// Compound index to prevent duplicate invitations to same email from same referral
referralSchema.index({ referFrdId: 1, 'toEmails': 1 });
