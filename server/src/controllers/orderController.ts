...
export const createOrder = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?._id;
    const { paymentMethod, billingInfo, shippingAddress } = req.body;

    console.log('Create order request:', { userId, paymentMethod, hasBilling: !!billingInfo, hasShipping: !!shippingAddress });
    ...
    // Payment method mapping to valid enum values
    const validPaymentMethods = {
      'credit-card': 'Credit Card',
      'debit-card': 'Debit Card', 
      'net-banking': 'Net Banking',
      'upi': 'UPI',
      'paypal': 'Credit Card', // Map PayPal to Credit Card
      'razorpay': 'Credit Card' // Map Razorpay to Credit Card
    };

    const mappedPaymentMethod = validPaymentMethods[paymentMethod] || 'Credit Card';
    ...
    // Get user for promo code validation
    const user = await User.findById(userId);
    if (!user) {
      return res.status(404).json({ message: 'User not found' });
    }

    // Use pre-calculated values from cart (cart already has discounts applied)
    const subtotal = cart.totalAmount; // Original total from cart
    const discountAmount = (cart as any).discountAmount || 0; // Pre-calculated discount from cart
    const subtotalAfterDiscount = (cart as any).subtotalAfterDiscount || subtotal; // Discounted subtotal from cart
    const shippingCharge = (cart as any).shippingCharge || 0; // Pre-calculated shipping from cart
    const totalBeforeTax = subtotalAfterDiscount + shippingCharge; // Total before tax from cart
    const gst = (cart as any).taxAmount || 0; // Pre-calculated tax from cart
    const totalAmount = (cart as any).finalTotal || subtotal; // Final total from cart

    console.log('Order totals from cart:', { 
      subtotal,
      discountAmount, 
    ...
    const orderItems = cart.items.map(item => {
      ...
      // Determine product model from product data
      let productModel = 'Pendant'; // Default fallback
      
      if (typeof item.product === 'object' && item.product) {
        // Check product category or type to determine model
        const productType = item.product.category || (item.product as any).type || item.product.name;
        
        if (productType?.toLowerCase().includes('earring')) {
          productModel = 'Earring';
        } else if (productType?.toLowerCase().includes('bracelet')) {
          productModel = 'Bracelet';
        } else if (productType?.toLowerCase().includes('ring')) {
          productModel = 'Ring';
        }
        // Default to 'Pendant' for other cases
      }
    ...
    // Create order with shipping address only
    const order = new OrderModel({
      user: userId,
      orderNumber: generateOrderNumber(),
      items: orderItems,
      ...
      paymentStatus: 'pending',
      orderStatus: 'pending',
      subtotal: subtotalAfterDiscount, // Discounted subtotal from cart
      gst,
      shippingCharge,
      totalAmount, // Final total from cart
    ...
    try {
      const user = await User.findById(userId);
      if (user) {
        // Handle referral code usage - add to usedReferralCodes array
        if (user.referredBy && user.refDiscount > 0 && !user.usedReferralCodes?.includes(user.referredBy)) {
          const referralCodeUsed = user.referredBy; // Store before adding to used list
          
          // Add to used referral codes array
          if (!user.usedReferralCodes) {
            user.usedReferralCodes = [];
          }
          user.usedReferralCodes.push(referralCodeUsed);
          
          // Clear the discount after using it
          user.refDiscount = 0;
          user.referredBy = null;
          await user.save();
          console.log('Referral discount marked as used and added to usedReferralCodes');
        ...
        } else if (user.referredBy && (user.refDiscount <= 0 || user.usedReferralCodes?.includes(user.referredBy))) {
        ...
        if (cart.appliedPromoCode) {
          ...
          if (promoCode && promoCode.isValid() && promoCode.canBeUsedBy(userId) && !user.usedPromoCodes.includes(promoCode._id)) {
