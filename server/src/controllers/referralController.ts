...
export const createReferral = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!toEmails || !Array.isArray(toEmails) || toEmails.length === 0) {
      console.error('Invalid toEmails:', toEmails);
    ...
    console.log('Fetching user with ID:', fromUserId);
    // Get user
    const user = await User.findById(fromUserId);
    if (!user) {
      console.error('User not found for ID:', fromUserId);
    ...
    console.log('User found:', { id: user._id, email: user.email, referralCode: user.referralCode });

    // Create referral record
    console.log('Creating referral record...');
    const referral = new Referral({
      referFrdId: user.referralCode,
      fromUserId,
      toEmails,
      note: note || "",
    ...
    await referral.save();
    console.log('Referral record saved:', referral._id);

    // Send invitation emails to all friends
    console.log(`Sending invitation emails to ${toEmails.length} recipients...`);
    ...
    console.log('=== Create Referral Complete ===');
  } catch (error) {
...
export const redeemPromoCode = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Special case: if code is 'SAVED', apply stored referral discount
    if (inputCode === 'SAVED') {
      ...
      if (currentUser.usedReferralCodes && currentUser.usedReferralCodes.includes(currentUser.referredBy)) {
        return res.status(400).json({ success: false, message: 'Referral discount has already been used' });
      }

      // Check if user has already used their referral discount
      if (currentUser.refDiscount <= 0) {
        return res.status(400).json({ success: false, message: 'Referral discount has already been used' });
      }
      ...
      // Don't clear the referral discount yet - only clear when order is created
    ...
    // Manual referral code input - compare with user's referredBy field
    const currentUser = await User.findById(userId);
    if (!currentUser) return res.status(404).json({ success: false, message: 'User not found' });

    // Check if the entered code matches the user's referredBy field
    if (currentUser.referredBy && currentUser.referredBy.toUpperCase() === inputCode.toUpperCase()) {
      // Check if user has already used this referral code
      if (currentUser.usedReferralCodes && currentUser.usedReferralCodes.includes(currentUser.referredBy)) {
        return res.status(400).json({ success: false, message: 'Referral discount has already been used' });
      }
      ...
      return res.json({ 
        success: true, 
        message: 'Referral code verified and discount applied', 
        data: { 
          discountAmount, 
          subtotal, 
          tax, 
