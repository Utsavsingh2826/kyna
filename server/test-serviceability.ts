...
async function testServiceabilityRoutes() {
  ...
  for (const pinCode of TEST_PIN_CODES.slice(0, 2)) {
    // Test only first 2 to avoid too many API calls
    try {
      ...
      if (response.ok && data.success) {
        log.success(
          `EDD for ${pinCode}: ${data.estimated_day || "N/A"} (${
            data.estimated_delivery || "No date"
          })`
        );
      } else {
...
async function testLegacyEDDController() {
  ...
  try {
    ...
    if (response.ok && data.success) {
      log.success(
        `Legacy EDD: ${data.estimated_delivery_day} (${data.estimated_delivery_date})`
      );
...
export {
  runAllTests,
  testServiceabilityRoutes,
  testLegacyEDDController,
  testInvalidInputs,
};
