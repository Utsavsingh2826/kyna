...
async function testFlexibleNamingConvention() {
  ...
  try {
    ...
    console.log(
      "📋 Testing Different Jewelry Types and Attribute Combinations:\n"
    );
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      try {
        ...
        const imageUrls = imageService.imageService.generateImageUrlsFlexible(
          testCase.sku,
          testCase.attributes
        );
        ...
        console.log(
          `   ✅ Generated Sub Images: ${imageUrls.sub.length} images`
        );
        console.log(
          `   ✅ Category: ${
            testCase.sku.startsWith("GR") ||
            testCase.sku.startsWith("ENG") ||
            testCase.sku.startsWith("SOL")
              ? "rings"
              : "other"
          }`
        );
    ...
    categoryTests.forEach((test) => {
      try {
        const imageService = require("./src/services/imageService.ts");
        const category = imageService.imageService.extractCategoryFromSku(
          test.sku
        );
        const status = category === test.expected ? "✅" : "❌";
        console.log(
          `   ${status} ${test.sku} -> ${category} (expected: ${test.expected})`
        );
      } catch (error) {
    ...
    console.log(
      "   ✅ Dynamic SKU support (GR1, ENG1, SOL1, BR1, PN1, ER1, etc.)"
    );
